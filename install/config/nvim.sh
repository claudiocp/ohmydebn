...
if [ ! -d $NVIM_CONFIG_DIR ]; then
  ~/.local/share/ohmydebn/bin/ohmydebn-headline "tte rain" "Configuring neovim with lazyvim"
...
if grep -q "colorscheme = \"catppuccin\"" $NVIM_PLUGINS/core.lua >/dev/null 2>&1; then
  ~/.local/share/ohmydebn/bin/ohmydebn-headline "tte rain" "Disabling old static neovim theme config"
  mv $NVIM_PLUGINS/core.lua $NVIM_PLUGINS/core.lua.disabled
fi

NVIM_THEME=$NVIM_PLUGINS/theme.lua
if [ ! -L $NVIM_THEME ]; then
  ~/.local/share/ohmydebn/bin/ohmydebn-headline "tte rain" "Configuring neovim theme"
  ln -snf ~/.config/ohmydebn/current/theme/neovim.lua $NVIM_THEME
fi

~/.local/share/ohmydebn/bin/ohmydebn-headline "tte rain" "Updating neovim config"
