...
PANEL_STATE=~/.local/state/ohmymint-panel
if [ ! -f $PANEL_STATE ]; then
  ~/.local/share/ohmymint/bin/ohmymint-headline "cat" "Configuring panel to be at top of screen"
  gsettings set org.cinnamon panels-enabled "['1:0:top']"
  mkdir -p ~/.local/state
  touch $PANEL_STATE
fi

PANEL_APPLET_STATE=~/.local/state/ohmymint-panel-applet
if [ ! -f $PANEL_APPLET_STATE ]; then
  ~/.local/share/ohmymint/bin/ohmymint-headline "cat" "Configuring panel applets"
...
WINDOW_SPEED_STATE=~/.local/state/ohmymint-window-speed
if [ ! -f $WINDOW_SPEED_STATE ]; then
  ~/.local/share/ohmymint/bin/ohmymint-headline "cat" "Setting Cinnamon window effect speed to maximum"
  gsettings set org.cinnamon window-effect-speed 2
  touch $WINDOW_SPEED_STATE
fi

ALTTAB_STATE=~/.local/state/ohmymint-alttab
if [ ! -f $ALTTAB_STATE ]; then
  ~/.local/share/ohmymint/bin/ohmymint-headline "cat" "Configuring Cinnamon alttab switcher"
...
SPICE_STATE=~/.local/state/ohmymint-spices
if [ ! -f $SPICE_STATE ]; then
  ~/.local/share/ohmymint/bin/ohmymint-headline "cat" "Configuring Cinnamon spices"
  for SPICE in "workspace-switcher@cinnamon.org" "notifications@cinnamon.org" "calendar@cinnamon.org"; do
    ...
    mkdir -p $SPICE_DIR
    ~/.local/share/ohmymint/bin/ohmymint-headline "cat" "Configuring Cinnamon spice $SPICE"
    cp -av ~/.local/share/ohmymint/config/cinnamon/spices/$SPICE/* $SPICE_DIR
    echo
  done
  touch $SPICE_STATE
fi

EXTENSIONS_STATE=~/.local/state/ohmymint-config/cinnamon-extensions-20250914
if [ ! -f $EXTENSIONS_STATE ]; then
  ~/.local/share/ohmymint/bin/ohmymint-headline "cat" "Configuring gTile extension"
  cd ~/.local/share/cinnamon/extensions/
  wget https://github.com/dougburks/gTile/releases/download/2.2.1/gTile.tar.gz
  tar zxvf gTile.tar.gz
  rm -f gTile.tar.gz
  cd - >/dev/null
  SPICE_DIR=~/.config/cinnamon/spices/gTile@OhMyMint
  mkdir -p $SPICE_DIR
  cp -av ~/.local/share/ohmymint/config/cinnamon/spices/gTile@OhMyMint/* $SPICE_DIR
  gsettings set org.cinnamon enabled-extensions "['gTile@OhMyMint']"
  mkdir -p ~/.local/state/ohmymint-config
