...
sudo apt update
sudo DEBIAN_FRONTEND=noninteractive apt -y install alacritty binutils btop chromium curl fzf git gimp golang gvfs-backends htop iperf3 keepassxc neovim openvpn pdftk-java python-is-python3 ripgrep screenfetch starship vim wget xdotool zsh zsh-autosuggestions zsh-syntax-highlighting
